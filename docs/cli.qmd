...
# Dequantize a quantized base to bf16 while merging
axolotl merge-lora config.yml --dequant
```

For quantized bases the default merge is format-preserving (the delta is folded and re-quantized back to the same format). `--dequant` writes a bf16 checkpoint instead; it is rejected for merge-aware NVFP4 adapters, where `--override-quantizer` can also be used to proceed past a quantizer-identity mismatch. See [NVFP4 MoE LoRA](nvfp4_lora.qmd).
...
Converts a GPT-OSS BF16 checkpoint (or a LoRA adapter + MXFP4 base) back to the original OpenAI MXFP4 weight-only format used by vLLM, SGLang, and TensorRT-LLM. Requires the `modelopt` extra (`nvidia-modelopt`). Docker images install this extra by default.

This is not [`axolotl quantize`](./quantize.qmd); torchao MXFP4 is a different checkpoint layout.

```bash
# Full fine-tune / already-merged BF16 checkpoint
...
See [the GPT-OSS examples](https://github.com/axolotl-ai-cloud/axolotl/tree/main/examples/gpt-oss) for the post-training workflow.
