...
#### Overriding Triton kernel options
...
`flex_attn_kernel_options` forces the given `kernel_options` (any of Triton's
`BLOCK_M`/`BLOCK_N`/`num_stages`/`num_warps`/etc.) onto every `flex_attention`
call, overriding whatever PyTorch's autotuner would have picked. It requires
`attn_implementation: flex_attention`.

This exists for one known failure mode: on H100, the autotuned config for some
shapes exceeds the 232KB per-SM shared-memory limit, and `flex_attention` OOMs
regardless of how much VRAM is free. The values above are one working
combination — with them, the Q/K/V tiles plus accumulator fit in roughly 80KB —
but they trade some throughput to get there, so only set this if you actually
hit the error; it is not a default and nothing enables it automatically. Tune
upward if your shapes leave headroom.

An explicit `kernel_options` passed at the call site (if some other integration
sets one) always wins over this default — the patch only fills it in when unset.
