...
::: {.callout-important}
For Blackwell GPUs, please use PyTorch ≥2.11.0 and CUDA 13.0. CUDA 12.8 cannot
compile for `sm_103a` (B300), and using CUDA 13.0 also avoids first-launch JIT
overhead on B100 / B200 / RTX 50-series.
:::
...
```{.bash}
export UV_TORCH_BACKEND=cu130  # or cu128
uv venv
source .venv/bin/activate
uv pip install --no-build-isolation axolotl[deepspeed]
```

Official Docker images also install the `modelopt` extra (`nvidia-modelopt`) so GPT-OSS MXFP4 export (`axolotl convert-gpt-oss-mxfp4`) works out of the box. For a local install that needs the same:

```{.bash}
uv pip install --no-build-isolation 'axolotl[deepspeed,modelopt]'
```
...
::: {.callout-important}
For Blackwell GPUs, please use `axolotlai/axolotl-uv:main-py3.12-cu130-2.12.0` or the cloud variant `axolotlai/axolotl-cloud-uv:main-py3.12-cu130-2.12.0`.
:::
