...
> A: Triton's autotuner picked a block-size config that exceeds H100's 232KB per-SM shared memory,
> independent of free VRAM. Force smaller blocks with `flex_attn_kernel_options` — see
> [Attention: Overriding Triton kernel options](attention.qmd#overriding-triton-kernel-options) for
> the exact values and tradeoffs.
