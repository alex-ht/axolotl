...
> A: We currently recommend torch 2.12 for use with `vllm`. Please ensure you use the right version. For Docker, please use a `-vllm` tagged image such as `main-py3.12-cu130-2.12.0-vllm`.
...
**Q: `flex_attention` OOMs on H100 with a Triton shared-memory error, even though there's plenty of VRAM free**

> A: Triton's autotuner can pick a block-size config whose working set exceeds H100's 232KB per-SM
> shared memory, independent of how much VRAM is free. Force smaller blocks:
>
> ```yaml
...
> ```
>
> This trades some throughput for headroom — with these values, SRAM usage for the Q/K/V tiles plus
> accumulator drops to roughly 80KB, well under the 232KB limit. Tune upward if your shapes have room.
