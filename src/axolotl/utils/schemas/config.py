...
class AxolotlInputConfig(
    ...
    def normalize_attn_implementation(cls, data):
        ...
        if data.get("gemma4_hybrid_attn_impl"):
            ...
            elif attn_impl and attn_impl not in _GEMMA4_HYBRID_ATTN_IMPLS:
                raise ValueError(
                    f"gemma4_hybrid_attn_impl requires attn_implementation to be one "
                    f"of {_GEMMA4_HYBRID_ATTN_IMPLS!r} (sliding-window layers run "
                    f"under it); got {attn_impl!r}."
                )
